...
description: "Safely operate the LazyCompass MongoDB CLI with a read-only-first workflow. Use when an AI agent needs to inspect `.lazycompass` config, read `lazycompass --help` output, resolve connection/database/collection context, run `lazycompass query`, `lazycompass agg`, or `lazycompass indexes`, capture results with `--table`, `--csv`, or `-o/--output`, or give brief user guidance for the TUI. Use for `insert`, `update`, or aggregation pipeline writes only when the user explicitly authorizes a database mutation."
---

# LazyCompass Operator

Operate LazyCompass with a CLI-first, read-only-first workflow. Prefer deterministic commands, `--help` discovery, and captured output over interactive TUI usage.
...
2. Prefer `lazycompass query`, `lazycompass agg`, and `lazycompass indexes` for agent work.
...
7. When command or flag behavior is unclear, read `lazycompass --help` or the specific subcommand help before guessing.
...
Prefer saved specs when they already exist. Otherwise run an inline `query` or `agg`, or `indexes` for collection index inspection. Pass `--connection` when multiple connections exist.
...
- `-o` works with JSON, table, and CSV output.

Resolution rules:

- If one connection exists, LazyCompass auto-selects it. If multiple exist, pass `--connection`.
- `query`, `agg`, `indexes`, `insert`, and `update` can omit `--db` when the selected connection has `default_database`.
...
- Root and subcommand `--help` output is a reliable summary of the current command and flag surface.
...
Only guide TUI usage when the user explicitly wants it. Keep guidance brief:

- `lazycompass` opens the TUI.
- Basic navigation: `j/k`, `h`, `l` or Enter, `?`, `q`.
- Read-only helpers: `r` run saved query, `a` run saved aggregation, `R` inline query, `S` inline aggregation, `I` show indexes, `x` export, `y` copy, `c` clear applied results.
- Local config/query helpers: `n` add connection, `Q` save query, `A` save aggregation.
