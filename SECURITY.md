...
- Automated install/upgrade flows always verify release checksums. If `gpg` is installed and a `.sha256.sig` asset is present, they also verify that signature against the bundled LazyCompass release key in an isolated GPG home. They do not trust unrelated keys from the user's default keyring.
- If a release ships both `.sha256` and `.sha256.sig`, verify the archive checksum and then verify the checksum signature against the bundled LazyCompass release key.
